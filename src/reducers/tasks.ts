  ...
  switch (type) {
    ...
    case actionTypes.MOVE_TASK:
      ...
      return {
        ...state,
        ...{
          [targetTask.type]: (state[targetTask.type] as Task<'TODO' | 'DOING' | 'DONE'>[]).filter(task => task.id !== targetTask.id),
          [listTypeWhereDragOver]: [...state[listTypeWhereDragOver], convertedTargetTask]
        }
      }
    case actionTypes.SAVE_TASKS:
      const { local } = storage;
      const taskNumber = Object.entries(state)
        .reduce((acc, [type, tasks]) => {
          local.remove(type)
